...
BumpOrBlast를 현재의 **2D 탑다운 슈터**에서 **뱀파이어 서바이버 계열 로그라이크**로 전환한다. 재사용 가능한 시스템(레벨업, 적 스폰, 드롭)을 최대한 살리고 필드·무기·진행 구조를 교체한다.

- **작성일**: 2026-04-21
- **최근 갱신**: 2026-05-12 — 자동전진·자동사격 폐기, 에너지·스탯·아이템·적 5등급 결정 추가
- **상태**: Phase R1·R2 코드 일부 반영 (회전 입력 추상화, 카메라 추적, EnemyRegistry, 탄창 제거)
- **관련 문서**: [GameDesign.md](GameDesign.md), [TechnicalSpec.md](TechnicalSpec.md)

## 변경 이력

### 2026-05-12 결정
1. **자동 전진 컨셉 폐기** → 기존 WASD 4방향 자유 이동 유지
2. **자동 사격 컨셉 폐기** → 기존 마우스 입력 사격 유지 (탄창은 폐기, fireRate 연사 유지)
3. **에너지 시스템 신설** (탄창 대체, 시간 자동 회복)
4. **중앙 PlayerStats + Modifier 구조 신설**
5. **아이템 시스템 신설** (영구 효과, 레벨업 선택지로 획득)
6. **드롭(Drop)/아이템(Item) 명명 분리** (기존 `ItemDropSystem` → `DropSystem` 리네임)
7. **적 5등급 색상 분류** (Gray/Green/Blue/Purple/Orange)
8. **아이템 5등급 색상 분류** (적과 동일 색계, 풀 구분 의미)
